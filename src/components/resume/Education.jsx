...
const Education = () => {
  return (
    <Section
      title="EDUCATION"
      icon={<GraduationCap className="w-4 h-4 sm:w-5 sm:h-5 text-cyan-400" />}
    >
      <div className="space-y-7 sm:space-y-9">
        {education.map((edu, idx) => (
          <motion.div
          ...
          >
            ...
            {/* Status */}
            <p className="mb-3 text-[10px] uppercase tracking-widest text-cyan-400/80">
              {edu.status === "Pursuing" ? "Currently Pursuing" : "Completed"}
            </p>
            ...
            {/* Divider */}
            <div className="my-3 h-px w-16 bg-cyan-400/40" />
            ...
            {/* Inner glow */}
            <div className="pointer-events-none absolute inset-0 shadow-[inset_0_0_28px_rgba(0,191,255,0.1)]" />
          </motion.div>
        ))}
      </div>
    </Section>
