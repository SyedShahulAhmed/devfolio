...
const Skills = () => (
  <Section
    title="CORE SKILLS"
    icon={<Code className="w-5 h-5 sm:w-6 sm:h-6 text-cyan-400" />}
  >
    <div className="space-y-6 sm:space-y-8">
      {Object.entries(skills).map(([cat, list], idx) => (
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ delay: idx * 0.05 }}
          className="relative border border-cyan-400/20 p-4 sm:p-5 bg-black/30"
        >
          {/* HUD corners */}
          ...
          {/* Inner glow */}
          <div className="pointer-events-none absolute inset-0 shadow-[inset_0_0_24px_rgba(0,191,255,0.08)]" />
        </motion.div>
      ))}
    </div>
  </Section>
