    ...
    <AnimatePresence>
      ...
      <motion.div
        className="
          ...
          p-6 sm:p-7
          shadow-[0_0_80px_rgba(0,191,255,0.18)]
        "
        initial={{ scale: 0.94, opacity: 0, y: 20 }}
        animate={{ scale: 1, opacity: 1, y: 0 }}
        exit={{ scale: 0.94, opacity: 0, y: 20 }}
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-cyan-400 hover:text-cyan-300 transition"
        >
          <X size={18} />
        </button>
        ...
        {/* HEADER */}
        <div className="mb-6">
          <div className="flex items-center gap-2 text-[10px] tracking-[0.3em] uppercase text-cyan-400">
            <ShieldCheck size={14} />
            Bridges — Verified Credential
          </div>
          ...
          <div className="mt-3 h-px w-24 bg-cyan-400/40" />
        </div>

        {/* CERTIFICATE FRAME */}
        <div className="relative border border-cyan-400/30 bg-black/60 p-3">
          <img
            src={cert.image}
            alt={cert.title}
            className="w-full object-contain"
          ...
          {/* Inner glow */}
          <div className="pointer-events-none absolute inset-0 shadow-[inset_0_0_30px_rgba(0,191,255,0.12)]" />
        </div>
