...
const Projects = () => (
  <Section
    title="PROJECT LOGS"
    icon={<FolderGit2 className="w-5 h-5 sm:w-6 sm:h-6 text-cyan-400" />}
  >
    <div className="space-y-7 sm:space-y-10">
      {projects.map((p, idx) => (
        <motion.div
        ...
        >
          ...
          {/* Meta row */}
          <div className="mb-3 flex items-center justify-between text-[10px] tracking-widest uppercase text-cyan-400/80">
            <span>// LOG_{String(idx + 1).padStart(2, "0")}</span>
            <span className="text-slate-500">{p.year}</span>
          </div>
          ...
          <h3 className="relative mb-3 text-base sm:text-lg font-semibold tracking-wide text-slate-100">
            {p.title}
            <span className="absolute -bottom-1 left-0 w-12 h-px bg-cyan-400/60" />
          </h3>

          {/* Description */}
          <p className="mb-4 text-xs sm:text-sm text-slate-400 leading-relaxed font-mono">
            <span className="text-cyan-400">//</span> {p.description}
          </p>

          {/* Highlights */}
          <div className="mb-4">
            <p className="mb-2 text-[10px] uppercase tracking-widest text-cyan-400">
              Functional Highlights
            </p>
            <ul className="space-y-1.5 text-[11px] sm:text-xs text-slate-400 leading-relaxed">
              {p.highlights.map((h) => (
            ...
            {p.deployLink && (
              <motion.a
                ...
                className="
                  ...
                  hover:shadow-[0_0_16px_rgba(0,191,255,0.25)]
                  transition
                "
              >
                <ExternalLink className="w-3 h-3" />
          ...
          {/* Inner glow */}
          <div className="pointer-events-none absolute inset-0 shadow-[inset_0_0_32px_rgba(0,191,255,0.1)]" />
        </motion.div>
      ))}
    </div>
  </Section>
